    ...
    if not multiple_project:
        ...
        for i in range(0, int(part_count)):
            ...
            if build_log is None:
                GlobalSettings.logger.debug('Part {0} not complete'.format(part_key))
...
def update_convert_log(s3_base_key, part=''):
    GlobalSettings.logger.debug(f"update_convert_log({s3_base_key}, {part!r})")
    build_log_json = get_build_log(s3_base_key, part)
    upload_convert_log(build_log_json, s3_base_key, part)
    return build_log_json
# end of update_convert_log function


def upload_convert_log(build_log_json, s3_base_key, part=''):
    GlobalSettings.logger.debug(f"upload_convert_log({build_log_json}, {s3_base_key}, {part!r})")
    ...
    cdn_upload_contents(build_log_json, build_log_key)
    return build_log_json
# end of upload_convert_log function


def cdn_upload_contents(contents, key):
    GlobalSettings.logger.debug(f"cdn_upload_contents({contents}, {key})")
...
def process_callback(pc_prefix, queued_json_payload, redis_connection):
    ...
    multiple_project = False # RJH: Not sure where/how to handle this yet
