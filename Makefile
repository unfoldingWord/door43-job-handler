...
runLocal: checkEnvVariables
	# This runs the rq job handler locally in a docker container
	#   which removes and then processes jobs from the local redis local- queue
	docker run -e QUEUE_PREFIX="local-" -e AWS_ENDPOINT -e DB_ENDPOINT -e TX_DATABASE_PW -e DEBUG_MODE=true -e REDIS_URL -e GOGS_USER_TOKEN -v ${PWD}:/scripts -v door43_u:/site/u --name D43_DevJob_Handler --rm --network "tx-net" python:3 /bin/bash -c "cd /scripts; pip install -r requirements.txt; rq worker --config rq_settings --name D43_Dev_JobHandler"
